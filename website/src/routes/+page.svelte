<div class="flex flex-row flex-wrap md:flex-no-wrap items-center justify-center">
	<div class="md:w-1/2 flex flex-col">
		<img src="/images/titletext.svg" alt="StoryStains" />
		<p class="logline text-xl text-center font-normal">An emotion driven story analysis app.</p>

		<div class="flex flex-col items-center justify-center mt-16 mb-8">
			<a class="mx-1 my-2" href="/android">
				<img
					alt="Get it on Google Play"
					src="/images/android-store-badge.png"
					style="height: 60px;"
				/>
			</a>
			<a class="mx-1 my-2" href="/apple">
				<img
					alt="Download on the App Store"
					src="/images/ios-store-badge.svg"
					style="height: 60px;"
				/>
			</a>
		</div>
	</div>

	<img src="/images/newicontest1.png" alt="" class="pt-12 px-4 md:w-1/2 md:pt-16 md:px-16" />
</div>

<section>
	<div class="container mx-auto flex px-4 py-24 md:flex-row flex-col items-center">
		<h3>Find the emotions of your stories.</h3>
		<p>Storystains allows you to analyze stories by their emotions.</p>
	</div>
</section>

<section>
	<div class="container mx-auto flex px-4 py-24 md:flex-row flex-col items-center">
		<h3>Breakdown stories into the important points.</h3>
		<p>StoryStains allows you to set what triggers your emotions.</p>
	</div>
</section>
